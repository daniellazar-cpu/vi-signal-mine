...
def test_the_word_corroborated_never_reaches_a_reader(screens):
    """The owner's exact reaction on meeting it was "wtf is Corroborated". It
    was invented in this codebase, so the count is the label instead — there is
    nothing to learn about "3 sources"."""
    c, paths = screens
    for name, path in paths.items():
        text = re.sub(r"<[^>]*>", " ", c.get(path).text)
        for word in ("Corroborated", "corroborated", "Emerging", "Single source"):
            assert word not in text, f"{name} still says {word!r}"


def test_the_source_count_is_shown_as_the_count(screens):
    c, paths = screens
    body = c.get(paths["insight"]).text
    assert 'class="pips"' in body, "no pip encoding for source counts"
    assert re.search(r'aria-label="\d+ independent source', body), (
        "the pips carry no accessible number"
    )


def test_one_table_never_states_the_same_fact_twice(screens):
    """The themes table carried a Tier badge *and* the source count. Those are
    the same fact in two columns, which is half of what "busy" meant."""
    c, paths = screens
    body = c.get(paths["insight"]).text
    head = re.search(r"<thead>(.*?)</thead>", body, re.S)
    assert head and "Tier" not in head.group(1), "the Tier column is back"
...
def test_the_source_vocabulary_has_one_source_of_truth():
    """The SVG, the tables and the generated documents must not each keep their
    own spelling of the same thing."""
    import vsm.modes.vocabulary as vocab
    from vsm.ui.app import _TIER_LABELS
    from vsm.ui.render import _TIER_LABEL

    assert _TIER_LABEL is vocab.SOURCE_LABEL
    assert _TIER_LABELS is vocab.SOURCE_LABEL


def test_no_screen_leaks_an_internal_word(tmp_path, monkeypatch):
    """The whole-app sweep. Every one of these was invented in this codebase or
    carries two meanings, and each reached a reader on at least one screen —
    including inside the document a client receives.

    Kept as one test over every surface because the vocabulary kept coming back
    through a route nobody was looking at: a browser tab title, an SVG axis
    label, a table caption, a section heading generated by the report writer.
    """
    monkeypatch.delenv("BLOB_READ_WRITE_TOKEN", raising=False)
    monkeypatch.delenv("VSM_ACCESS_KEY", raising=False)
    ts = TopicStore(tmp_path / "db")
    ...
    c = TestClient(create_app(topic_store=ts, run_store=rs))
    t = ts.list()[0]
    by = {m: [r for r in rs.for_topic(t.topic_id, m)] for m in ("mine", "insight", "report")}
    paths = {
        "overview": "/", "topics": "/topics", "how": "/how",
        "deliverables": "/deliverables", "topic": f"/topics/{t.topic_id}",
        "confirm": f"/topics/{t.topic_id}/confirm",
        "snapshot": f"/runs/{by['mine'][-1].run_id}/snapshot",
        "insight": f"/runs/{by['insight'][-1].run_id}/insight",
    ...
    for name, path in paths.items():
        ...
        if hit:
            leaks[name] = hit
    assert not leaks, f"internal vocabulary reached the reader: {leaks}"
